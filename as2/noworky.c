...
void tradeArrays(double *array1, double *array2, int size)
{
	unsigned int i;
	for (i = size-1; i > 0; i--) { 
		swapContent(array1+i, array2+i); //bug was here: it was iterating i down passed 0 into negatives, and arrays were trying to access negative indexes.  Should be stopping the loop at iteration 0.
	}
}
